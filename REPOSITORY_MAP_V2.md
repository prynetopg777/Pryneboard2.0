...
### Core Infrastructure (`odysseus(Pryne)/core/`)
...
### Business Logic (`odysseus(Pryne)/src/`)
...
### MCP Servers (`odysseus(Pryne)/mcp_servers/`)
...
### Frontend (`odysseus(Pryne)/static/js/`)
...
## 4. Entry Points
- **Web App:** `odysseus(Pryne)/app.py` (FastAPI).
...
### Refactor Candidates
- **`odysseus(Pryne)/static/app.js`**: Massive monolithic file (~5k+ lines). High risk of regression. 
    - *Action:* Phase 3 will migrate this to React components.
- **`odysseus(Pryne)/app.py`**: Route registration is manual and bloated.
...
- `odysseus(Pryne)/src/rag_manager.py`: Legacy single-stage RAG logic that should be superseded by `rag_vector.py`.
