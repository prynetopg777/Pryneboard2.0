...
def _migrate_add_content_hash_columns():
    """Add content_hash to documents and memories for deduplication. Idempotent."""
    import sqlite3
    db_path = DATABASE_URL.replace("sqlite:///", "")
    if not os.path.exists(db_path):
        return
    try:
        ...
        # Documents
        cursor = conn.execute("PRAGMA table_info(documents)")
        if "content_hash" not in [row[1] for row in cursor.fetchall()]:
            conn.execute("ALTER TABLE documents ADD COLUMN content_hash VARCHAR(64)")
            conn.execute("CREATE UNIQUE INDEX IF NOT EXISTS ux_documents_owner_hash ON documents(owner, content_hash)")
            logging.getLogger(__name__).info("Migrated: added unique 'content_hash' index to documents")
        
        # Memories
        cursor = conn.execute("PRAGMA table_info(memories)")
        if "content_hash" not in [row[1] for row in cursor.fetchall()]:
            conn.execute("ALTER TABLE memories ADD COLUMN content_hash VARCHAR(64)")
            conn.execute("CREATE UNIQUE INDEX IF NOT EXISTS ux_memories_owner_hash ON memories(owner, content_hash)")
            logging.getLogger(__name__).info("Migrated: added unique 'content_hash' index to memories")
        
        conn.commit()
        conn.close()
    except Exception as e:
        logging.getLogger(__name__).warning(f"content_hash migration failed: {e}")


def _migrate_add_session_namespace_column():
    """Add namespace column to sessions table if it doesn't exist."""
    import sqlite3
    db_path = DATABASE_URL.replace("sqlite:///", "")
    if not os.path.exists(db_path):
        return
    try:
        ...
        if "namespace" not in columns:
            conn.execute("ALTER TABLE sessions ADD COLUMN namespace TEXT")
            conn.execute("CREATE INDEX IF NOT EXISTS ix_sessions_namespace ON sessions(namespace)")
            conn.commit()
            logging.getLogger(__name__).info("Migrated: added 'namespace' column to sessions")
        conn.close()
    except Exception as e:
        logging.getLogger(__name__).warning(f"Migration check for namespace failed: {e}")
...
def init_db():
    ...
    _migrate_add_session_namespace_column()
    ...
    _migrate_add_content_hash_columns()
