    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = 'Pryneboard';
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    if (role === 'assistant' && metadata && metadata.reasoning_content) {
       const thinkBlock = document.createElement('details');
       thinkBlock.className = 'agent-tool-output agent-thought-block';
       thinkBlock.innerHTML = `<summary>Thought</summary><pre>${uiModule.esc(metadata.reasoning_content)}</pre>`;
       box.appendChild(thinkBlock);
    }
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'Pryneboard' : shortModel(resolvedModel);
