...
def setup_personal_routes(personal_docs_manager, rag_manager, rag_available):
    ...
    @router.post("/api/ingest/discord")
    async def ingest_discord(
        request: Request,
        body: Dict[str, Any],
        owner: str = Depends(require_user),
    ):
        """Trigger ingestion of a pre-parsed Discord log file."""
        from src.integrations.discord_harvester import ingest_discord_file
        from src.personal_docs import _resolve_allowed_personal_dir

        file_path = body.get("file_path")
        namespace = body.get("namespace", "discord-general")
        
        if not file_path:
            raise HTTPException(400, "Missing file_path")
            
        try:
            # Re-use existing path resolution logic
            # The harvester expects a file path, we ensure it's within allowed personal dirs
            safe_path = _resolve_allowed_personal_dir(file_path)
            
            if not os.path.exists(safe_path):
                 raise HTTPException(404, "File not found")
                 
            result = ingest_discord_file(safe_path, owner=owner, namespace=namespace)
            return result
        except Exception as e:
            logger.error(f"Error triggering discord ingestion: {e}")
