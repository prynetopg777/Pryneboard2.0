...
from src.app.document_helpers import (
...
def setup_document_routes(session_manager, upload_handler=None) -> APIRouter:
    ...
    async def import_pdf(
        ...
        MIN_CONTENT_THRESHOLD = 50 # characters
        if not body_text or len(body_text.strip()) < MIN_CONTENT_THRESHOLD:
            logger.warning(f"[Precision Engine] Blocking ingestion of low-content PDF for user {user}: {title} ({len(body_text or '')} chars)")
            raise HTTPException(400, f"Document has insufficient readable text (min {MIN_CONTENT_THRESHOLD} chars required)")

        # Precision Engine: Deduplicate by content hash before creating document
        from src.document_processor import calculate_content_hash
        content_hash = calculate_content_hash(body_text or "")
        
        if content_hash:
            db = SessionLocal()
            try:
                # Check for existing document with this hash and owner
                existing = db.query(Document).filter(
                    Document.content_hash == content_hash,
                    Document.owner == user
                ).first()
                if existing:
                    logger.info(f"[Precision Engine] Duplicate PDF content detected for user {user}: {title}")
                    return {**_doc_to_dict(existing), "is_duplicate": True}
            finally:
                db.close()
    ...
    async def prepare_signed_reply(doc_id: str, request: Request):
        ...
        try:
            ...
            try:
                from src.app.email_routes import _imap, _decode_header
            except Exception:
                _imap = None
