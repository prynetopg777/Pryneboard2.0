...
class FastEmbedReranker:
    """Local reranking client using fastembed (ONNX).
    Uses Cross-Encoder models to re-score retrieval candidates for higher precision.
    """

    def __init__(self, model: Optional[str] = None):
        try:
            from fastembed import TextReranking
        except ImportError as e:
            raise RuntimeError(
                "Local reranking needs 'fastembed'. Run: pip install fastembed"
            ) from e

        # Use a high-quality lightweight reranker by default
        self.model = model or os.getenv("RERANK_MODEL", "BAAI/bge-reranker-base")
        ...
        self._reranker = TextReranking(model_name=self.model, cache_dir=cache_dir)
        logger.info(f"FastEmbed Reranker loaded model={self.model}")

    def rerank(self, query: str, documents: List[str]) -> List[float]:
        """Score a list of documents against a query. Returns scores [0-1]."""
        if not documents:
            return []
        
        # TextReranking.rerank returns an iterable of objects with a .score attribute
        results = self._reranker.rerank(query, documents)
        return [float(r.score) for r in results]


def get_reranker_client():
    """Factory: get a local reranker client.
    Note: Reranking is computationally expensive and defaults to CPU/local.
    """
    try:
        client = FastEmbedReranker()
        return client
    except Exception as e:
        logger.warning(f"Reranker initialization failed: {e}. Retrieval will use raw vector similarity.")
        return None
