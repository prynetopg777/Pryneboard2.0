...
def _generate_doc_id(text: str, owner: str = "") -> str:
    # Use full 64-char hash for 2.0 Precision Engine
    key = f"{owner}\x00{text}" if owner else text
    return f"doc_{hashlib.sha256(key.encode('utf-8')).hexdigest()}"
...
class VectorRAG:
    ...
    def _initialize_system(self) -> bool:
        try:
            ...
            from src.embeddings import get_embedding_client, get_reranker_client
            from core.constants import RERANK_ENABLED
    ...
    def add_document(self, text: str, metadata: Dict[str, Any]) -> bool:
        ...
        # Precision Engine: Guard against empty or low-value content
        MIN_CONTENT_THRESHOLD = 50 
        if len(text.strip()) < MIN_CONTENT_THRESHOLD:
            logger.warning(f"[VectorRAG] Ingestion blocked: Content too short ({len(text)} chars)")
            return False
        ...
        try:
            ...
            # 1. ID check (Collision/Atomic dedupe)
            # Check both 2.0 (64-char) and 1.0 (16-char) IDs to prevent re-indexing legacy data
            legacy_id = f"doc_{hashlib.sha256((f'{owner}\\x00{text}' if owner else text).encode('utf-8')).hexdigest()[:16]}"
            existing = self._collection.get(ids=[doc_id, legacy_id])
            if existing["ids"]:
                return True

            # 2. Semantic dedupe via content_hash (if provided)
            if content_hash:
                # Query across this owner's documents
                hash_matches = self._collection.get(
                    where={"content_hash": content_hash},
                    include=["metadatas"]
                )
                if hash_matches and hash_matches['ids']:
                    for meta in hash_matches['metadatas']:
                        if meta.get("owner") == owner:
                            logger.info(f"[VectorRAG] Ingestion skipped: Duplicate content_hash {content_hash} for owner {owner}")
                            return True
    ...
    def add_documents_batch(self, docs: List[tuple]) -> Dict[str, Any]:
        ...
        valid = [
            (t, m) for t, m in docs
            if t and isinstance(t, str) and len(t.strip()) >= 50 and m and isinstance(m, dict)
        ]
        if not valid:
    ...
    def search(self, query: str, k: int = 5, owner: Optional[str] = None, namespace: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Sort by initial hybrid similarity
            candidates.sort(key=lambda c: c["similarity"], reverse=True)

            # Stage 2: Cross-Encoder Reranking
            if self._reranker and candidates:
                logger.info(f"[Precision Engine] Reranking {len(candidates)} candidates for query: {query[:50]}...")
                try:
                    # Reranking is sync/blocking, but we are inside an async route thread pool usually
                    texts = [c["document"] for c in candidates]
                    rerank_scores = self._reranker.rerank(query, texts)
                    
                    for i, score in enumerate(rerank_scores):
                        candidates[i]["rerank_score"] = round(score, 4)
                        # Blend or replace similarity? The manifest says "improve context relevance".
                        # Replacing similarity with rerank_score for the top selection.
                        candidates[i]["similarity"] = candidates[i]["rerank_score"]
                    
                    # Re-sort by rerank score
                    candidates.sort(key=lambda c: c["similarity"], reverse=True)
                    logger.info(f"[Precision Engine] Reranking complete. Top score: {candidates[0]['similarity']}")
                except Exception as re:
                    logger.warning(f"Reranking failed: {re}. Falling back to hybrid scores.")

            top = candidates[:k]
            logger.info(f"Retrieval for '{query[:60]}': {len(top)} results")
            return top
